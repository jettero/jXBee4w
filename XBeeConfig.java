...
public class XBeeConfig {
    ...
    public static int config(CommPortIdentifier port, int speed, boolean force) {
        ...
        try {
            ...
            try {
                ...
                // not a debug message
                System.out.print("firmware revision: " + res[res.length-1] + " ");
    ...
    public String[] config(String settings[], Pattern expect[]) throws IOException, XBeeConfigException {
        ...
        // 0000000: 4f4b 0dff 7e00 028a 0174                 OK..~....t

        String res = new String(send_and_recv("ATFR\r")); // restart under the new settings

        // modem status watchdog timer reset
        byte modem_status[] = { 0x7e, 0x00, 0x02, (byte) 0x8a, 0x01, 0x74 };

        if( !( res.startsWith("OK") && res.endsWith(new String(modem_status)) ) ) {
